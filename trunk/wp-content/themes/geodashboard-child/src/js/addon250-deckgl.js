const a250_slug='DeckGLOnClick';

dyn_functions['addon250-deckgl'+'_ready'] = function(){

  // _onsole.log('addon250-deckgl'+'_ready');
  ...
  list_mapclick.push(a250_slug);

  prepare_a250();

}

function prepare_a250(){
...
dyn_mapclick[a250_slug] = function(e){
  ...
  if(objAddon.properties.mapclick_status=='enabled'){
    ...
    let item_dlg = 'dlg_a250_DeckGLOnClick';
...
dyn_functions['template_by_slug_'+'dlg_a250_DeckGLOnClick'+'_single'] = function(){

  let dlg_slug = 'dlg_a250_DeckGLOnClick'+'_single';
  ...
  // a250_lyr.clearLayers();

  // var marker = L.marker([lat, lng], {
  //   icon: a250_lyr_Icon
  // });

  // a250_lyr.addLayer(marker);
  // a250_lyr.addTo(mymap);
...
function get_deckGL_geometries(datastring){
  ...
  dataString.collection='viewBuildings3d-2';
...
function a250_buildingColor(mycategory){
...
dyn_functions['succ_fill_deckGL'] = function(r){
  ...
  const geojsonLayer = new GeoJsonLayer({
    ...
    getFillColor: f => a250_buildingColor(f.properties.mycategory),
    getLineColor: f => [255, 255, 255],
  
    pickable: true,
    onClick: (info, event) => a250_onClickBuilding(info, event) //console.log('growth:', info.object.properties)
  });
...
function a250_onClickBuilding(info, event){
